...
function initAcademy() {
  ...
  const previousBtn = document.getElementById("previous-card-btn");
  ...
  let state = { arcana: "major", suit: "all", query: "", index: 0, flipped: false };
  ...
  function filteredCards() {
    return TAROT_CARDS.filter(matches);
  }
  ...
  function render() {
    ...
    if (filtered.length === 0) {
      resultsCount.textContent = "Không tìm thấy lá bài phù hợp";
      progressBar.style.width = "0";
      stage.innerHTML = '<div class="no-results">Không tìm thấy lá bài nào khớp với "' + escapeHTML(state.query) + '".</div>';
      return;
    }

    state.index = Math.max(0, Math.min(state.index, filtered.length - 1));
    const card = filtered[state.index];
    resultsCount.textContent = "Lá " + (state.index + 1) + " / " + filtered.length + " · chạm để lật";
    progressBar.style.width = ((state.index + 1) / filtered.length * 100) + "%";
    stage.innerHTML = [
        '<div class="flash-card study-card' + (state.flipped ? ' flipped' : '') + '" data-id="' + card.id + '" tabindex="0" role="button"',
        ' aria-label="Lật lá ' + escapeHTML(card.name_en) + ' để xem ý nghĩa">',
        '<div class="flash-card-inner">',
        '<div class="flash-face flash-front">',
        '<img src="' + card.image + '" alt="Hình minh hoạ lá ' + escapeHTML(card.name_en) + '">',
        '<div class="flash-front-label">',
        '<div class="flash-front-sub">' + escapeHTML(card.name_vi || "") + '</div>',
    ...
    const studyCard = stage.querySelector(".flash-card");
    const flip = function () { state.flipped = !state.flipped; studyCard.classList.toggle("flipped", state.flipped); };
    studyCard.addEventListener("click", flip);
    studyCard.addEventListener("keydown", function (e) {
  ...
  previousBtn.addEventListener("click", function () { changeCard(-1); });
  nextBtn.addEventListener("click", function () { changeCard(1); });
  randomBtn.addEventListener("click", function () {
    const filtered = filteredCards();
    if (filtered.length < 2) return;
    let nextIndex = state.index;
    while (nextIndex === state.index) nextIndex = Math.floor(Math.random() * filtered.length);
